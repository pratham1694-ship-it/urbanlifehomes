...
void main(){
  ...
  if (uUseImage == 1) {
    vec2 imgUV = gl_FragCoord.xy / uResolution;
    imgUV.y = 1.0 - imgUV.y;
    vec4 imgSample = texture2D(uImage, imgUV);
    pixelColor = imgSample.rgb;

    feed = fbm2(uv, uTime * 0.05);
    feed = feed * 0.5 - 0.2;
    feed = feed + (uDensity - 0.5) * 0.3;
  } else {
    feed = fbm2(uv, uTime * 0.05);
    feed = feed * 0.5 - 0.2;
    feed = feed + (uDensity - 0.5) * 0.3;
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      if (imageSrc) {
        ...
        imageTexture = loader.load(imageSrc);
